...
class MainPageBannerItem extends StatefulWidget {
  ...
  MainPageBannerItem({this.index = 0,this.currPageValue = 0, Key? key }) : super(key: key);

  @override
  State<MainPageBannerItem> createState() => _MainPageBannerItemState();
}

class _MainPageBannerItemState extends State<MainPageBannerItem> with AutomaticKeepAliveClientMixin{
  ...
  Matrix4 getTransMatrix4(){
    ...
    if(widget.index == widget.currPageValue.floor()){
      currScale = 1 - (widget.currPageValue - widget.index) * (1 - _scaleFactor);
    }else if(widget.index == widget.currPageValue.floor() + 1){
      currScale = _scaleFactor + (widget.currPageValue - widget.index + 1) * (1 - _scaleFactor);
    }else if(widget.index == widget.currPageValue.floor() - 1){
      currScale = 1 - (widget.currPageValue - widget.index) * (1 - _scaleFactor);
    }else{
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        image: DecorationImage(
          image: NetworkImage(MockImage.imgId(widget.index + 200)),
          fit: BoxFit.cover
        )
      ),
    );
